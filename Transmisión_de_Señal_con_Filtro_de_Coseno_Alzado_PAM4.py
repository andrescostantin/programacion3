...
def generar_senal_PAM4(num_simbolos=100):
    # Genera una señal aleatoria de -3, -1, 1, y 3 (niveles PAM4)
    return np.random.choice([-3, -1, 1, 3], size=num_simbolos)
...
def preparar_interfaz():
    ventana = tk.Tk()
    ventana.title("Simulación de Transmisión de Señal con Filtro de Coseno Alzado - PAM4")
    ...
    def simular():
        ...
        # Generar señal con PAM4
        senal = generar_senal_PAM4(num_simbolos)
        ...
        graficar_senal(fig, ax1, np.arange(len(senal)), senal, 'Señal Original PAM4')
        graficar_senal(fig, ax2, t, senal_filtrada, 'Señal Filtrada PAM4')
